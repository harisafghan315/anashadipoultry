...
export default function Inventory() {
  ...
  const initialTab = ['medicine', 'meel'].includes(searchParams.get('tab')) ? searchParams.get('tab') : 'medicine'
  const [tab, setTab] = useState(initialTab)
  // Sync tab when the URL query param changes (e.g. navigating in from Roznamcha quick-entry).
  useEffect(() => {
    const qt = searchParams.get('tab')
    if (qt && ['medicine', 'meel'].includes(qt) && qt !== tab) setTab(qt)
  ...
  function openAdd() {
    setEditItem(null)
    setForm({ ...emptyForm, type: tab, unit: UNITS[tab]?.[0] || emptyForm.unit })
