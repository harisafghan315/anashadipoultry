...
import { EXPENSE_CATEGORY_BI } from '../utils/biLabels'
...
export default function Expenses() {
  ...
  async function handleSubmit(e) {
    ...
    if (editTarget) {
      await updateExpense(editTarget.id, { ...form, amount })
    } else {
      const created = await addExpense({ ...form, amount })
      if (created && fromStoreCash && amount > 0) {
        await recordOut({ amount, source: 'expense', reference_id: created.id, note: form.title, date: form.expense_date })
      }
    }
    setSaving(false)
    setModalOpen(false)
  ...
  async function handleDelete(expense) {
    ...
    await removeByReference(expense.id)
  }
    ...
    { key: 'title', label: t('expenses.title'), render: r => <span className="font-medium text-slate-700">{lf(r, 'title', lang)}</span> },
    {
      ...
      render: r => (
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${CAT_COLORS[r.category] || CAT_COLORS.other}`}>
          {CAT_ICONS[r.category]} {EXPENSE_CATEGORY_BI[r.category] || r.category}
        </span>
      )
    },
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!editTarget && (
            <label className="flex items-center gap-2 text-sm text-slate-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2 cursor-pointer">
              <input type="checkbox" checked={fromStoreCash} onChange={e => setFromStoreCash(e.target.checked)} className="rounded" />
