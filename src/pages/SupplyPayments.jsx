...
const bilingualSupply = item => SUPPLY_ITEM_BI[item] || item
...
export default function SupplyPayments() {
  ...
  function openAdd() { setEditTarget(null); setForm(emptyForm); setFromStoreCash(true); setModalOpen(true) }
  ...
  async function handleSubmit(e) {
    ...
    setSaving(false)
    if (ok) {
      if (!editTarget && fromStoreCash) {
        const amt = parseFloat(form.amount) || 0
        if (amt > 0) await recordOut({ amount: amt, source: 'supply_payment', reference_id: ok.id, note: supplyItem, date: form.payment_date })
      }
  ...
  async function handleDelete() {
    ...
    await removeByReference(deleteTarget.id)
