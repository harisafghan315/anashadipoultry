...
export default function POS() {
  ...
  const [paidToStoreCash, setPaidToStoreCash] = useState(true)
      ...
      if (customerType === 'farm') {
        ...
        if (result) {
          if (paid > 0) {
            ...
            if (paidToStoreCash && payRow) {
              await recordIn({ amount: payAmt, source: 'payment', reference_id: payRow.id, note: lf(farms.find(f => f.id === farmId), 'name', lang), date: saleDate })
            }
          }
        ...
        if (sale) {
          if (paid > 0 && paidToStoreCash) {
            await recordIn({ amount: paid, source: 'sale', reference_id: sale.id, note: customerName, date: saleDate })
          }
          setReceipt({
      ...
      <div className="lg:w-72 flex flex-col gap-4">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100 space-y-4">
          ...
          <div>
            ...
            {paid > 0 && (
              <label className="mt-2 flex items-center gap-2 text-sm text-slate-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 cursor-pointer">
                <input type="checkbox" checked={paidToStoreCash} onChange={e => setPaidToStoreCash(e.target.checked)} className="rounded" />
